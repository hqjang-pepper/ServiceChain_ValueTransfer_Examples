...
async function deploy_ERC20(info) {
    ...
    try {
        ...
        // Deploy ERC20 token
        const instancetoken = new caver.klay.Contract(tokenAbi);
        info.newInstance = await instancetoken.deploy({data: tokenCode, arguments:[info.newInstanceBridge._address]})
            .send({ from: info.sender, gas: 100000000, value: 0 })
...
(async function TokenDeploy() {
    await deploy_ERC20(conf.scn)
    await deploy_ERC20(conf.en)
    ...
    await conf.en.newInstance.methods.addMinter(conf.en.bridge).send({ from: conf.en.sender, to: conf.scn.bridge, gas: 100000000, value: 0 })
